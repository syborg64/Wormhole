...
impl Provider {
    ...
    pub fn rmfile(&mut self, parent_ino: u64, name: &OsStr) -> io::Result<()> {
        let file =
            self.filesystem_from_parent_ino_and_name(parent_ino, name, FileType::RegularFile)?;
    ...
    //NOTE - Can be only remove an empty folder.
    pub fn rmdir(&mut self, parent_ino: u64, name: &OsStr) -> io::Result<()> {
        let folder =
            self.filesystem_from_parent_ino_and_name(parent_ino, name, FileType::Directory)?;
        match self.fs_readdir(folder.0) {
            Ok(files_system) => {
                if files_system.len() > 0 {
                    return Err(io::Error::new(
                        io::ErrorKind::NotFound,
                        "This is not a folder is not empty",
                    ));
                }
