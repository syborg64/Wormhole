...
impl DiskManager for UnixDiskManager {
    ...
    fn new_file(&self, path: &WhPath, mode: u16) -> io::Result<()> {
        self.handle.new_file(path.clone().set_relative(), mode.into())?;  // TODO look more in c mode_t value
        Ok(())
    }
    ...
    fn read_file(&self, path: &WhPath, offset: usize, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let mut read = file.bytes()
        .skip(offset)
        .take(buf.len())
        .map_while(|b| b.ok()).collect::<Vec::<u8>>();
    ...
    fn new_dir(&self, path: &WhPath, permissions: u16) -> io::Result<()> {
        self.handle.create_dir(path.clone().set_relative(), permissions.into()) // TODO look more in c mode_t value
    }
